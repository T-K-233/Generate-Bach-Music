...
def main():
    ...
    if args.evaluate:
        ...
        cvt_chrmid_2_csv(sample)
        cvt_csv_2_midi()

def cvt_chrmid_2_csv(string):
    string = string.split(' ')
    keys = []
    prev_keys = []
    t = 0
    with open('output/temp.csv', 'w', newline='') as f:
        writer = csv.writer(f)
...
def cvt_csv_2_midi():
    time.sleep(2)
    os.system("dependencies\Csvmidi.exe .\\output\\temp.csv .\\output\\result.mid")
